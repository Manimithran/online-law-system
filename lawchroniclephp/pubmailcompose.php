...
?>
                 ...
                 <form name="f" action="pubmailcompose.php" method="post" enctype="multipart/form-data" onsubmit="return check()">
<table align="center" border="0">
<tr>
<td align="right">To</td>

<td><input type="text" name="t1" size="30">
<a href="lawlist.php" name="ll">Lawyers List</a>
</td>
</tr>
 <tr>
...
if(isset($_POST['submit'])) {
	include("dbcon.php");
	$types=array("image/pjpeg","image/png","image/gif","image/jpeg","text/plain","application/pdf");
	$flag=false;
	$from=$_SESSION['un'];
	$to=$_POST['t1'];
	$to=explode(',',$to);
	...
	foreach($to as $tt) {
	$tt=trim($tt);
	$rs=mysqli_query($conid,"select refno from lawyerreg where lname='$tt'") or die(mysqli_error($conid));
	$rn=mysqli_fetch_array($rs);
	$sdn=$rn[0];
	$sdt=date("Y-m-d h:i:s",time()+19800);
	if(mysqli_num_rows($rs)>0) {
	mysqli_query($conid,"insert into inbox values('$usn','$sdn','$sub','$sdt','$content','$target')") or die(mysqli_error());
